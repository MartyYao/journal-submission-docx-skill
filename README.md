...
## v0.1.1 更新说明（2026-08-11）

**表头渲染修复（三轮，用户五点）**：
- ①**跨列合并去重 bug**：prefix 合并循环禁止用 `j = end` 改 for 循环变量（range 迭代器继续产生重复 j，已合并列被 else 分支再写一次——"变量变量"/"行政保守化行政保守化"），必须 **while 循环 + `j = end + 1`**；此 bug 同时是"表头重复写三次"与"SE 行行高异常"的根因
- ②**表头字号**：`hsize = min(数据字号, 8.5pt)`，窄列表头单行不换行
- ③**表格后间距**：表格后紧跟的正文/标题段落 space_before +6pt（与表注留一行空间，`space_before_extra` 参数）
- ④SE 行（括号标准误）与系数行等高（随 ① 修复）
- ⑤表注行距均匀（表注单元格 line=1.0 统一）

**分页修复（发布前实测发现）**：
- 隔离空段创建顺序修正：必须在标题段**之前**（原实现在标题段之后创建，隔离失效导致 LO 吞 5 个章节标题——4/9 页首）
- markdown 分隔线（`---`）跳过（原来渲染为文本段）

**验证**：重新生成投稿版 9/9 章节页首 + 验证脚本 7/7 通过。
