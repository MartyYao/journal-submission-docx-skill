...
def add_para(doc, text, style, bold=False, italic=False, keep_with_next=False):
    ...
    if header:
        for j in range(ncol):
            ...
            if end > j:
                ...
                set_cell(merged, prefixes[j], size, bold=True)
            else:
                set_cell(table.cell(0, j), prefixes[j], size, bold=True)
            j = end
        for j in range(ncol):
            set_cell(table.cell(1, j), suffixes[j], size, bold=True, align_left=(j == 0))
...
def parse_and_build(md_dir, out_docx):
    ...
    def para(text, style, **kw):
        """add_para 包装：纯纵向方案（无分节）；文档第一段显式纵向节已不需要"""
